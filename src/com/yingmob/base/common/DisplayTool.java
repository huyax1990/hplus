package com.yingmob.base.common;
	...
	public Map<String, Object> displayUser() {
		...
		if (cacheResult != null) {
			...
			for (SysUserModel user : users) {
				result.put(user.get("id") + "", user.getStr("nick"));
			}
			putCache(USER_KEY, result);
		}
		return result;
	}
	...
	public Map<String, Object> displayRole() {
		...
		if (cacheResult != null) {
			...
			for (SysRolesModel role : roles) {
				result.put(role.get("id") + "", role.getStr("roleName"));
			}
			putCache(ROLE_KEY, result);
		}
		return result;
	}
	...
	public Map<String, Object> displayPost() {
		...
		if (cacheResult != null) {
			...
			for (SysPostsModel post : posts) {
				result.put(post.get("id") + "", post.getStr("postName"));
			}
			putCache(POST_KEY, result);
		}
		return result;
	}

	public Map<String, String> displayConfig(String key) {
