...
public class SysConfigModel extends Model<SysConfigModel> {
	...
	public SysConfigModel getConfig(String key, String field) {
		List<SysConfigModel> SysConfigModelList = dao.findByCache(EhcacheConstants.SYS_CONFIG, key + "_" + field, "select id,`key`,field,val from sys_config where `key`=? and field=? and state=1",
				key, field);
